...
public class DeviceInsertTask extends AbstractConsumerTask {
	...
	public void execute(TaskInfo data) {
		...
		try {
			...
			InsertDeviceQMsg qmsg = JsonUtils.convertJsonToObject(body, InsertDeviceQMsg.class);
			deviceService.insertDevice(qmsg);
			...
			sbLog.insert(0, "Insert device failed!");
			LOG.error(sbLog.toString(), e);
			data.setStatus(TaskStatus.FAILED);
		}
	}
